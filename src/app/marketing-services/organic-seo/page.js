...
export default function OrganicSEO() {
  ...
  const packageFeatures = ["16 hours per month x 3", "Site Audit", "Topic & Keyword Research", "SEO Plan", "On-Page Optimisation", "Internal Link Building", "External Link Building", "SEO Optimised Content Generation"]
        ...
        <main className="flex min-h-min mt-24 md:my-0 flex-col w-full items-center justify-center pattern-cross pattern-green-600 pattern-bg-white pattern-size-6 pattern-opacity-20">
          ...
          <ColumnsTwoFullH>
            ...
            <section className="flex flex-col gap-8">
              <h2 className="font-bold text-xl">My past two projects resulted in...</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center">
                <p className="text-teal-500 text-4xl lg:text-5xl font-bold text-center">{`> 1000 % Growth`}</p>
                <p className="text-xs text-gray-600">For a technology start-up, I grew their organic web traffic from under 200 monthly visitors to over 10,000, generating over 1,200 leads in just 12 months. </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                <p className="text-teal-500 text-4xl lg:text-5xl font-bold text-center">{`125 % Growth`}</p>
                <p className="text-xs text-gray-600">An established B2B food manufacturer saw their web visitors increase from 2,000 monthly visitors to 4,500 visitors in under 12 months and exponentially improved lead generation. </p>
              </div>
            </section>
          </ColumnsTwoFullH>
          <section className="bg-teal-100 rounded-xl p-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 lg:gap-20 w-11/12 mx-auto md:w-5/6 items-center mb-20">
            <div className="flex flex-col gap-4">
              <h2 className="text-3xl font-bold w-full text-gray-800">
                The Key Is{" "}
                <span className="bg-gradient-to-br from-teal-500 to-indigo-600 text-transparent bg-clip-text">
          ...
          <Packages description={packageDescription} title={packageTitle} features={packageFeatures} cost="£1,800" />
          <PeterWilliams />
